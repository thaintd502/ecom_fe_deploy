  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:9090/admin/get-all-customers', {
          headers: {
        ...
        const customersWithAddress = await Promise.all(customersData.map(async customer => {
          const addressResponse = await axios.get(`http://localhost:9090/admin/customer-address/${customer.customerId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
