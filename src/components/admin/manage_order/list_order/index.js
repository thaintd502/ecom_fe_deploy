...
const OrderList = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:9090/admin/get-all-orders', {
          headers: {
  ...
  const handleDelete = (orderId) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa đơn hàng này không?')) {
      ...
      axios.delete(`http://localhost:9090/admin/delete-order/${orderId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleStatusChange = (orderId, newStatus) => {
    axios.put(`http://localhost:9090/admin/update-order-status/${orderId}`, { status: newStatus }, {
      headers: {
